...
RockPaperScissors adalah aplikasi web yang mampu membedakan gambar yang terlihat pada kamera (webcam) pengguna. Gambar yang dapat dibedakan adalah Batu (Rock), Kertas (Paper), Gunting (Scissors). Dengan kemampuan pengolahan data Computer Vision pada platform browser.
...
│   │   ├── app-model.js    # Kelas untuk menangani pemuatan model TensorFlow.js.
...
│   │   ├── model/          # Folder berisi file model Machine Learning.
│   │   │   ├── model.json  # Struktur model.
│   │   │   ├── weights.bin # Bobot model.
│   │   │   └── metadata.json # Metadata model, seperti nama label kelas.
│   │   └── utils/          # Berisi fungsi-fungsi pembantu (utilities).
│   │       └── camera.js   # File untuk mengelola akses webcam dan pemrosesan frame.
